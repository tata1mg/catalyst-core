...
async function createKeystore(projectPaths, androidConfig) {
    ...
    if (
        keystoreConfig.storePassword === "your_store_password" ||
        ...
        throw new Error(
            'Please update keystore passwords in config file. Do not use placeholder values like "your_store_password" or "your_key_password".'
        )
    }
...
async function initializeConfig(configPath) {
    ...
    if (android.createSignedAAB) {
        ...
        if (android.keystoreConfig) {
            ...
            if (
                android.keystoreConfig.storePassword === "your_store_password" ||
                ...
                progress.log(
                    `Warning: Keystore contains placeholder passwords. Please update them.`,
                    "warning"
                )
            } else {
...
async function diagnoseAndroidProject(projectPath) {
    ...
    const hasRootBuildFile = diagnostics.foundFiles.some(
        (f) => f.files.includes("build.gradle") || f.files.includes("build.gradle.kts")
    )
    const hasAppBuildFile = diagnostics.foundFiles.some(
        (f) => f.files.includes("app/build.gradle") || f.files.includes("app/build.gradle.kts")
    )
...
async function validateProjectStructure(androidConfig) {
    ...
    if (diagnostics.projectType === "incomplete") {
        throw new Error(
            `Android project appears to be incomplete. Missing critical files: ${diagnostics.missingFiles.map((f) => f.description).join(", ")}`
        )
    }

    if (diagnostics.projectType !== "standard-android") {
        progress.log(
            `Warning: Project type "${diagnostics.projectType}" may not be fully compatible`,
            "warning"
        )
    }
    ...
    if (currentProjectDir !== oldProjectName) {
        progress.log(
            `Warning: Current directory name "${currentProjectDir}" differs from old project name "${oldProjectName}"`,
            "warning"
        )
    }
                ...
                if (files.trim()) {
                    ...
                    for (const file of fileList) {
                        try {
                            ...
                            progress.log(
                                `Warning: Could not update file ${file}: ${fileError.message}`,
                                "warning"
                            )
                        }
                    }
...
async function buildAndroidAAB(configPathOrConfig) {
    ...
    try {
        ...
        const keystorePath = await createKeystore(
            { ...projectPaths, newProjectPath: tempDeploymentPath },
            androidConfig
        )
        ...
        if (keystorePath && androidConfig.createSignedAAB) {
            ...
            aabResult = await createSignedAAB(
                { ...projectPaths, newProjectPath: tempDeploymentPath },
                androidConfig,
                keystorePath
            )
            ...
            if (aabResult) {
                ...
                aabResult = await finalizeAABAndCleanup(
                    projectPaths,
                    androidConfig,
                    aabResult,
                    tempDeploymentPath
        ...
        progress.printTreeContent("Build Summary", [
            ...
            {
                text: `Original project: ${projectPaths.oldProjectPath}`,
                indent: 1,
                prefix: "├─ ",
                color: "gray",
            },
            {
                text: `Project name: ${androidConfig.oldProjectName} → ${androidConfig.newProjectName}`,
                indent: 1,
                prefix: "├─ ",
                color: "gray",
            },
            {
                text: `Deployment path: ${androidConfig.deploymentPath || "Not specified"}`,
                indent: 1,
                prefix: "├─ ",
                color: "gray",
            },
            {
                text: `Overwrite existing: ${androidConfig.overwriteExisting}`,
                indent: 1,
                prefix: "├─ ",
                color: "gray",
            },
            ...
            {
                text: `Keystore: ${keystorePath ? "Created/Verified" : "Skipped"}`,
                indent: 1,
                prefix: "├─ ",
                color: "gray",
            },
            {
                text: `Signed AAB: ${aabResult ? "Created" : "Skipped"}`,
                indent: 1,
                prefix: "└─ ",
                color: "gray",
            },
            ...
            "Output Files:",
            {
                text: `Original preserved at: ${projectPaths.oldProjectPath}`,
                indent: 1,
                prefix: "├─ ",
                color: "green",
            },
            ...(backupPath
                ? [{ text: `Backup: ${backupPath}`, indent: 1, prefix: "├─ ", color: "gray" }]
                : []),
            ...(aabResult
                ? [
                      {
                          text: `AAB file: ${aabResult.finalAabPath}`,
                          indent: 1,
                          prefix: "├─ ",
                          color: "green",
                      },
                      {
                          text: `AAB size: ${aabResult.fileSize} MB`,
                          indent: 1,
                          prefix: "└─ ",
                          color: "green",
                      },
        ...
        if (progress.currentStep) {
            ...
            progress.printTreeContent("Troubleshooting Guide", [
                ...
                {
                    text: "Verify config file exists and contains required android configuration",
                    indent: 1,
                    prefix: "├─ ",
                    color: "yellow",
                },
                {
                    text: "Ensure project path exists and is accessible",
                    indent: 1,
                    prefix: "├─ ",
                    color: "yellow",
                },
                ...
                {
                    text: "Verify no processes are using the project directory",
                    indent: 1,
                    prefix: "├─ ",
                    color: "yellow",
                },
                {
                    text: "Ensure sufficient disk space for project copy",
                    indent: 1,
                    prefix: "├─ ",
                    color: "yellow",
                },
                {
                    text: 'Update keystore passwords (do not use "your_store_password")',
                    indent: 1,
                    prefix: "└─ ",
                    color: "yellow",
                },
                ...
                {
                    text: `Config: ${typeof configPathOrConfig === "string" ? configPathOrConfig : "Direct object from buildAppAndroid.js"}`,
                    indent: 1,
                    prefix: "├─ ",
                    color: "gray",
                },
                {
                    text: `Project path: ${androidConfig?.projectPath || "Not loaded"}`,
                    indent: 1,
                    prefix: "├─ ",
                    color: "gray",
                },
                {
                    text: `Deployment path: ${androidConfig?.deploymentPath || "Not loaded"}`,
                    indent: 1,
                    prefix: "├─ ",
                    color: "gray",
                },
                {
                    text: `Old project name: ${androidConfig?.oldProjectName || "Not loaded"}`,
                    indent: 1,
                    prefix: "├─ ",
                    color: "gray",
                },
                {
                    text: `New project name: ${androidConfig?.newProjectName || "Not loaded"}`,
                    indent: 1,
                    prefix: "├─ ",
                    color: "gray",
                },
                {
                    text: `Overwrite existing: ${androidConfig?.overwriteExisting !== undefined ? androidConfig.overwriteExisting : "Not loaded"}`,
                    indent: 1,
                    prefix: "└─ ",
                    color: "gray",
                },
                ...
                {
                    text: "Consistent store and key passwords for PKCS12",
                    indent: 1,
                    prefix: "└─ ",
                    color: "cyan",
                },
