...
export const translateError = (nativeError) => {
    ...
    if (errorString.includes('permission') && errorString.includes('denied')) {
        ...
        if (errorString.includes('storage') || errorString.includes('external_storage')) {
            return createStandardError(ERROR_CODES.PERMISSION_DENIED, 'Storage permission denied', nativeError);
        }
    ...
    // Network errors
    if (errorString.includes('network') || errorString.includes('connection') || errorString.includes('internet')) {
    ...
    if (errorString.includes('activitynotfound')) {
        return createStandardError(ERROR_CODES.FEATURE_UNSUPPORTED, 'No app available to handle this request', nativeError);
    }
