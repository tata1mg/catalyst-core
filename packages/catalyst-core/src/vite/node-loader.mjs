...
const createDirnameShim = (source) => {
    const hasFilename = /\b(?:const|let|var)\s+__filename\b/.test(source)
    const hasDirname = /\b(?:const|let|var)\s+__dirname\b/.test(source)
    if (hasFilename && hasDirname) return ""

    return [
        !hasFilename && `import { fileURLToPath as __node_loader_fileURLToPath } from "url";`,
        !hasDirname && `import { dirname as __node_loader_dirname } from "path";`,
        !hasFilename && `const __filename = __node_loader_fileURLToPath(import.meta.url);`,
        !hasDirname && `const __dirname = __node_loader_dirname(__filename);`,
    ]
        .filter(Boolean)
        .join("\n")
}
