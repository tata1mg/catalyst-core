...
function createAssetsPhase(ctx) {
    ...
    async function processNotifications(WEBVIEW_CONFIG) {
        ...
        try {
            await writeGeneratedBuildProperties(hasNotificationConfig)
    ...
    async function writeGeneratedBuildProperties(notificationsEnabled) {
        try {
            const propertiesPath = `${pwd}/androidProject/app/catalyst-build.properties`
            const propertiesContent = [
                "# Generated by Catalyst. Used by Android Studio builds when -PconfigPath is not provided.",
                `notifications.enabled=${notificationsEnabled ? "true" : "false"}`,
                "",
            ].join("\n")
