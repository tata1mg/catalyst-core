...
/** A captured photo, however it reached the web layer. */
export interface CapturedPhoto {
    /** Displayable source for the image. */
    fileSrc: string
    /** Name the file was given. */
    fileName: string
    /** Size in bytes, or 0 when the native side did not report it. */
    size: number
    /** MIME type of the image. */
    mimeType: string
    /** How the payload crossed the bridge. */
    transport: string
    /** What produced the photo. */
    source: string
}

/** What {@link useCamera} returns. Wraps one operation: capturing a photo. */
export interface UseCameraResult extends HookState<CapturedPhoto>, WebFallbackState {
    /** Progress of the in-flight capture, or null under SSR. */
    progress: HookProgress | null
    /** Capture a photo. The canonical action. */
    takePhoto: () => void
    /** Alias of `takePhoto`, named to match the shared hook contract. */
    execute: () => void
    /** Drop the captured photo and any error. */
    clear: () => void
    /** Drop the current error, keeping any photo. */
    clearError: () => void
    /** Native camera permission status, or null on web. */
    permission: string | null
    /**
     * The captured photo.
     *
     * @deprecated Use `data` instead. `photo` is a deprecated alias kept for
     ...
     * Drop the captured photo and any error.
     *
     * @deprecated Use `clear` instead. `clearPhoto` is a deprecated alias kept
     * for backward compatibility and will be removed at 2.0.
     */
    clearPhoto: () => void
}

export const useCamera = ({ webFallback }: WebFallbackOptions = {}): UseCameraResult => {
...
/**
 * What {@link useCameraPermission} returns.
 *
 * Gained `loading`, `data`, `error`, `execute`, `isNative` and `isWeb` when the
 * hook surface was made consistent; the older key for each is kept as an alias.
 */
export interface UseCameraPermissionResult extends HookState<string>, WebFallbackState {
    /** Request camera access. The canonical action. */
    execute: () => Promise<void> | void
    /**
     * The permission status.
     *
     * @deprecated Use `data` instead. `permission` is a deprecated alias kept
     * for backward compatibility and will be removed at 2.0.
     */
    permission: string | null
    /**
     * True while the permission state is resolving.
     *
     * @deprecated Use `loading` instead. `isLoading` is a deprecated alias kept
     * for backward compatibility and will be removed at 2.0.
     */
     ...
     *
     * @deprecated Use `execute` instead. `request` is a deprecated alias kept
     * for backward compatibility and will be removed at 2.0.
     */
    request: () => Promise<void> | void
}

export const useCameraPermission = ({ webFallback }: WebFallbackOptions = {}): UseCameraPermissionResult => {
    ...
    const permResultRef = useRef<any>(null)
