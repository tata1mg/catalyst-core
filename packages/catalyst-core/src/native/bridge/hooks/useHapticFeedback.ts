...
export const useHapticFeedback = ({ webFallback }: WebFallbackOptions = {}) => {
    ...
    // The caller passes `(type, options)`, but the web path has never read the
    // second argument — `navigator.vibrate` takes only a pattern. The parameter
    // is declared so the discard is explicit at the signature rather than being
    // an arity mismatch; behavior is unchanged.
    const handleWebHaptic = (type: string, _options?: any) => {
