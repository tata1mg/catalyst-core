...
import type { HookEnvironment, HookProgress, HookState } from "../useBaseHook.js"

// Shared callback system — allows hook + requestNotificationPermission to share one WebBridge listener
const permissionStatusListeners = new Set<(data: any) => void>()
let isPermissionCallbackRegistered = false

const registerPermissionStatusListener = (callback: (data: any) => void) => {
 ...
 *
 * Gained `loading`, `data`, `error`, `isNative` and `isWeb` when the hook
 * surface was made consistent, and it now reports permission failures as a
 * standard error object instead of only logging them.
 */
export interface UseNotificationPermissionResult extends HookState<string> {
    /**
...
/**
 * What {@link useNotification} returns.
 *
 * This is the one multi-action hook that still carries `execute`. It and
 * `schedule` are frozen aliases of `scheduleLocal` kept for backward
 * compatibility, and neither should be read as the pattern for multi-action
 * hooks — those expose their named functions and no `execute` at all.
 */
export interface UseNotificationResult extends HookState {
    /** Progress of the in-flight operation, or null under SSR. */
    progress: HookProgress | null
    /** Drop the current result and error. */
    clear: () => void
    /** Drop the current error, keeping any data. */
    clearError: () => void
    /** Latest notification permission status, or null. */
    permissionStatus: string | null
    /** Push token issued by the platform, or null. */
    pushToken: string | null
    /** Current app badge count. */
    badges: number
    /** The most recently received or tapped notification, or null. */
    lastNotification: any
    /** Topics this device is subscribed to. */
    subscribedTopics: any[]
    /** Schedule a local notification. The canonical action. */
    scheduleLocal: (config: any) => void
    ...
    /** Ask the native side for the current topic subscriptions. */
     ...
     *
     * @deprecated Use `scheduleLocal` instead. `execute` is a frozen alias kept
     * for backward compatibility and will be removed at 2.0.
     */
     ...
     * Schedule a local notification.
     *
     * @deprecated Use `scheduleLocal` instead. `schedule` is a frozen alias kept
     * for backward compatibility and will be removed at 2.0.
     */
