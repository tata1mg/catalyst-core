...
import type { HookProgress, HookState, WebFallbackOptions, WebFallbackState } from "../useBaseHook.js"
...
/**
 * What {@link useFilePicker} returns. Wraps one operation: picking a file.
 *
 * The web-fallback keys are declared required: every client branch returns
 * them, and consumers should not have to null-check keys that never miss at
 * runtime. The SSR stub under-delivers on this shape (it omits
 * `webFallbackActive`, `webFallbackDisabled` and `setWebFallback`) and is cast
 * below — reconciling the stub itself is a behavior change that belongs to the
 * freeze, not to this conversion.
 */
export interface UseFilePickerResult extends HookState, WebFallbackState {
    /** Progress of the in-flight pick, or null under SSR. */
    progress: HookProgress | null
    /** Open the file picker. The canonical action. */
    pickFile: (input?: any) => void
    /** Alias of `pickFile`, named to match the shared hook contract. */
    execute: (input?: any) => void
    /** Drop the picked files, the cached File objects and any error. */
    clear: () => void
    /** Drop the current error, keeping any data. */
    clearError: () => void
    /** Materialize the picked file as a `File`. */
    getFileObject: (...args: any[]) => Promise<any>
    /** Materialize every picked file as a `File`. */
    getAllFileObjects: (...args: any[]) => Promise<any>
    /** Whether the current file's transport supports `File` creation. */
    canCreateFileObject: boolean
    /** Per-file view of `canCreateFileObject`. */
    canCreateFileObjects: any[]
    /** First picked file, or null. */
    selectedFile: any
    /** Every picked file. */
    selectedFiles: any[]
    /** The current phase of the pick, or null when idle. */
    ...
    isLoading: boolean
    /**
     * Drop the picked files and any error.
     *
     * @deprecated Use `clear` instead. `clearFile` is a deprecated alias kept
     * for backward compatibility and will be removed at 2.0.
     */
    clearFile: () => void
}

export const useFilePicker = ({ webFallback }: WebFallbackOptions = {}): UseFilePickerResult => {
    const base = useBaseHook("useFilePicker", { hasWebFallback: true, webFallback })
    const fileObjectCache = useRef(new Map<any, any>())
    ...
    if (typeof window === "undefined") {
        // The stub under-delivers on UseFilePickerResult (no web-fallback keys).
        return SSR_FILE_PICKER_STUB as unknown as UseFilePickerResult
    }
