...
import type { WebFallbackOptions } from "../useBaseHook.js"
...
/** Options accepted when starting the native or web video stream. */
export interface VideoStreamStartOptions {
    /** Which camera to open: `"back"` or `"front"`. */
    facing?: string
    /** Detection filter: `"qr"`, `"barcode"` or `"all"`. */
    format?: string
    /** Zoom behaviour, e.g. `{ auto: true, initial: 1.0 }`. */
    zoom?: any
    /** Target frame rate. */
    fps?: number
    [key: string]: any
...
/** Options accepted by {@link useVideoStream}. */
export interface VideoStreamOptions extends WebFallbackOptions {
    /** Called with each detected QR/barcode payload. */
    onQRDetected?: (value: any) => void
}

export const useVideoStream = ({ onQRDetected, webFallback }: VideoStreamOptions = {}) => {
    ...
    const webStart = useCallback(
        async (options: VideoStreamStartOptions = {}) => {
        ...
        async (type, value) => {
            ...
            switch (type) {
                ...
                case "fps": {
                    ...
                    const { min = null, max = null } = value as { min?: number | null; max?: number | null }
                    const frameRate: any = {}
