...
/** Transition options, accepted as hook defaults and per-navigate overrides. */
export interface NativeTransitionOptions {
    /** Animation style. Defaults to `"slide"`. */
    type?: "slide" | "fade"
    /** Slide direction. Slide only. Defaults to `"left"`. */
    direction?: "left" | "right" | "up" | "down"
    /** Animation length in ms. Defaults to 300. */
    duration?: number
    /** Safety timer in ms. Defaults to `max(duration * 3, 800)`. */
    timeout?: number
    /** Replace the current history entry instead of pushing. */
    replace?: boolean
    /** Router state to carry through the navigation. */
    state?: any
}

/**
 * What {@link useNativeTransition} returns.
 *
 * A multi-action hook: its named functions are the API, so it has no `execute`.
 * It also returns no `error` key — a failed transition self-heals into a plain
 * navigation, so there is nothing for a caller to handle.
 */
export interface UseNativeTransitionResult extends HookEnvironment {
    /** Navigate with a native transition. */
    navigate: (to: any, options?: NativeTransitionOptions) => void
    /** Cancel an in-flight transition. Best-effort. */
     ...
     * True while a transition is running.
     *
     * @deprecated Use `loading` instead. `transitioning` is a deprecated alias
     * kept for backward compatibility and will be removed at 2.0.
     */
...
export const useNativeTransition = (defaults: NativeTransitionOptions = {}): UseNativeTransitionResult => {
    ...
    const webTimeoutRef = useRef<any>(null)
    const pendingCommitRef = useRef<any>(null)
    ...
    const navigate = useCallback(
        (to: any, options: NativeTransitionOptions = {}) => {
            const {
