...
class NativeBridgeUtil {
    /**
     * Platform detection, resolved once at construction.
     *
     * Declared without initializers so they erase at compile time — the
     * constructor below stays the only thing that assigns them.
     *
     * These are a snapshot; `isAvailable()` re-detects live for callers that
     * must not read a stale value.
     */
    isAndroid: boolean
    isIOS: boolean
    ...
    file = {
         ...
         */
        pick: (options: string | Record<string, any> | null = "*/*") => {
            if (options == null) {
