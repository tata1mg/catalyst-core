...
/** Options accepted when opening the native camera. */
export interface CameraOptions {
    ...
    cameraDevice?: string
    /** Flash behaviour: `"auto"`, `"on"` or `"off"`. */
    flashMode?: string
    /** Whether to let the user edit the image after capture. */
    allowEditing?: boolean
}

/** Configuration accepted when requesting camera permission. */
export interface CameraPermissionConfig {
    /** Ask the native side for a detailed permission payload. */
...
class CameraUtils {
    ...
    static openCamera(
        ...
        registerCallback: (event: string, handler: (data: any) => void) => void,
     ...
     */
    static validateCameraOptions(options: CameraOptions): CameraOptions {
        const validated: CameraOptions = {}
     ...
     */
    static isValidDevice(device: string): boolean {
     ...
     */
    static isValidFlashMode(flashMode: string): boolean {
