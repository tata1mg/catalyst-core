 ...
 *   fetch / XMLHttpRequest                                 → NetworkTimingCollector
 *
...
import { PerfStore } from "./core/store.js"
...
import { NetworkTimingCollector } from "./collectors/NetworkTimingCollector.js"

class WebPerfCollector {
    // Internal collector state, assigned in the constructor. The perf module is
    // internal to the bridge and deliberately left loosely typed during the
    // migration, so an index signature stands in for per-field declarations.
    [key: string]: any
    constructor() {
        ...
        this._store = new PerfStore(nativeToWeb)
        ...
        this._networkTiming = new NetworkTimingCollector(measure)
    ...
    static init(bridge) {
        ...
        console.log("[CatalystPerf] active — nativeOffset:", window.__NATIVE_TIME_OFFSET ?? 0, "ms")
    }
        ...
        window.__catalystPerfBatch = (batchId, jsonArrayString) => {
            ...
            try {
                console.log(
                    ...
                    typeof jsonArrayString === "string" ? jsonArrayString.length : "?"
                )
                const events =
                ...
                console.log(
                    "[CatalystPerf] __catalystPerfBatch events:",
                    events.length,
    ...
    _drainBatch(events) {
        ...
        const processChunk = (deadline) => {
            let processed = 0
            while (index < events.length && processed < CHUNK) {
    ...
    _routeBatchEvent(event) {
        const { type } = event
        this._store.recordNativeEvent(event, "native-batch")
    ...
    _routeNativeEvent(event) {
        this._store.recordNativeEvent(event, "native-live")
        switch (event.type) {
            case "keyboard-show":
