...
export class HookCollector {
    ...
    _closeLazySpan() {
        ...
        this._notifyWaterfall?.({
            hookType: "lazy-load",
            label: `Hook lazy-load ${p.target}`,
            startTime: p.startTime,
            endTime,
            durationMs: Math.round(endTime - p.startTime),
            loafCount: p.loafCount,
            totalBlockingMs: Math.round(p.totalBlocking),
            likelyCause: "lazy-load",
        })
    ...
    _emitLoopSpan() {
        ...
        this._notifyWaterfall?.({
            hookType: "computation-loop",
            label: "Hook computation loop",
            startTime: first.start,
            endTime: last.end,
            durationMs: Math.round(last.end - first.start),
            loafCount: this._loopLoafs.length,
            totalBlockingMs: Math.round(totalBlocking),
            likelyCause: "computation-loop",
        })
