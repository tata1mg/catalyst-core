...
export class RenderCollector {
    ...
    _observeLoAF() {
        ...
        const obs = new PerformanceObserver((list) => {
            // long-animation-frame entries carry blockingDuration/scripts, which the
            // base PerformanceEntry type in lib.dom does not declare.
    ...
    _observeLayoutShift() {
        ...
        const obs = new PerformanceObserver((list) => {
            // layout-shift entries carry hadRecentInput/value, which the base
            // PerformanceEntry type in lib.dom does not declare.
            for (const entry of list.getEntries() as any[]) {
