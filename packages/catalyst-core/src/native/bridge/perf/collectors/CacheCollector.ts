...
export class CacheCollector {
    ...
    _emitCacheSpan(event, track) {
        ...
        const endTime =
            nativeStartMs != null ? this._nativeToWeb(nativeStartMs) + (durationMs ?? 0) : performance.now()
    ...
    _emitSummarySpan(event) {
        ...
        const color = hitRatePct >= 80 ? COLOR.PRIMARY_DARK : hitRatePct >= 50 ? COLOR.SECONDARY : COLOR.ERROR
