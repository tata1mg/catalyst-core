...
export class Measure {
     ...
     * @param {string} [trackGroup] - Chrome DevTools track group
     */
    emit(name, startMark, endTime, detail = {}, track, color = null, trackGroup = TRACK_GROUP) {
        try {
    ...
    _colorFor(name, detail, startTime, endTime) {
        ...
        if (name.startsWith(PREFIX.SESSION_SCROLL)) return detail.loafCount > 0 ? COLOR.ERROR : COLOR.PRIMARY
