    ...
    async function generatePackageSwift(pluginDependencies = []) {
        try {
            ...
            pluginDependencies.forEach((d) =>
                mergePackageDependency(coreDependencyMap, d, "plugin manifests")
            )
            const coreDependencies = [...coreDependencyMap.values()].sort((l, r) =>
            ...
            notificationsDependencies.forEach((d) =>
                mergePackageDependency(packageDependencyMap, d, "notifications target")
            )
            const packageDependencies = [...packageDependencyMap.values()].sort((l, r) =>
                l.url.localeCompare(r.url)
            )
            ...
            try {
                ...
                /* hash file absent — proceed with update */
            }
...
${formatSwiftProductEntries(notificationsDependencies).join(",\n")}
            ...
            path: "Sources/CatalystNotifications"
                ...
                try {
                    ...
                    if (isNotificationsEnabled) {
                        ...
                        progress.log(
                            `Warning: Package resolution may have failed: ${error.message}`,
                            "warning"
                        )
                    }
                }
    ...
    async function addPluginResourcesToXcodeProject(resources) {
        try {
            ...
            fs.writeFileSync(projectFilePath, projectContent, "utf8")
            progress.log(
                `Registered ${resources.length} managed plugin resource(s) in Xcode project`,
                "success"
            )
        } catch (error) {
