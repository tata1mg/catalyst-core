...
module.exports = function createAssetsPhase(ctx) {
    ...
    async function handleGoogleServicesPlist() {
        try {
            ...
            const rootPath = `${process.cwd()}/GoogleService-Info.plist`
            if (error.code === "EACCES" && fs.existsSync(rootPath))
    ...
    function validateNotificationAsset(filePath, assetType) {
        ...
        else if (assetType === "sound" && fileSizeKB > 1024)
            progress.log(
            ...
            for (const icon of NOTIFICATION_ICONS) {
                for (const format of imageFormats) {
                    ...
                    if (fs.existsSync(iconImagePath)) {
                        ...
                        const contentsJson = {
                            images: [
                                { filename: `${icon.sourceName}.${format}`, idiom: "universal", scale: "1x" },
    ...
    async function removeSoundFilesFromXcodeProject() {
        try {
            ...
            for (const sound of NOTIFICATION_SOUNDS) {
                ...
                const regexes = [
                    new RegExp(
                        `\\t\\t[A-F0-9]+ \\/\\* ${p}\\.[a-z0-9]+ \\*\\/ = \\{isa = PBXFileReference;[^}]*\\};\\n`,
                        "g"
                    ),
                    new RegExp(
                        `\\t\\t[A-F0-9]+ \\/\\* ${p}\\.[a-z0-9]+ in Resources \\*\\/ = \\{isa = PBXBuildFile;[^}]*\\};\\n`,
                        "g"
                    ),
            ...
            progress.log(
                `Warning: Could not remove sound files from Xcode project: ${error.message}`,
                "warning"
            )
        }
    }
        ...
        try {
            ...
            for (const ext of imageExtensions) {
                ...
                if (fs.existsSync(sourcePath)) {
                    ...
                    const contentsJson = {
                        images: [{ filename: `launchscreen.${ext}`, idiom: "universal", scale: "1x" }],
                        info: { author: "xcode", version: 1 },
                    }
            ...
            if (!splashImageFound) {
                ...
                progress.log(
                    "Supported formats: splashscreen.png, splashscreen.jpg, splashscreen.jpeg",
                    "info"
                )
            }
        } catch (error) {
                ...
                try {
                    for (const item of fs.readdirSync(dir)) {
                        ...
                        if (stat.isDirectory())
                            ...
                            stat.isFile() &&
                            extensions.includes(path.extname(item).toLowerCase().slice(1))
                        )
            ...
            for (const { size, idiom, scale } of iconSizes) {
                ...
                for (const ext of imageExtensions) {
                    const matchingImage = allImages.find(
                        (imgPath) => path.basename(imgPath, `.${ext}`) === expectedName
                    )
                    if (matchingImage) {
