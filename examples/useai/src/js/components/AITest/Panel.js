...
export default function Panel({
    ...
    return (
        <div
            className={`flex flex-col flex-1 min-w-0 bg-[var(--surface)] border ${borderColor} rounded-2xl overflow-hidden shadow-xl`}
        >
            {/* header */}
            <div className="shrink-0 border-b border-[var(--border)] bg-[var(--surface-2)] px-5 py-3.5 flex items-center justify-between select-none">
                <div className="flex items-center gap-2.5">
                    ...
                    <div>
                        ...
                        <div className={`font-mono text-[10px] ${accentColor}`}>{endpointLabel}</div>
                    </div>
                </div>
                <div className="flex items-center gap-2">
                    ...
                    {sessionMode === "stateful" && useAIResult.conversationId && (
                        <span
                            className="px-2 py-0.5 text-[10px] font-semibold bg-indigo-500/10 text-indigo-300 border border-indigo-500/20 rounded-md font-mono"
                            title={useAIResult.conversationId}
                        >
                            session active
                        </span>
                    )}
                    ...
                    <span
                        className={`font-mono text-[11px] ${streaming ? "text-[var(--teal)]" : loading ? "text-yellow-400" : "text-[var(--text-3)]"}`}
                    >
                        {statusLabel}
                    </span>
                </div>
                ...
                <div className="shrink-0 px-5 py-2 bg-[var(--surface-2)] border-b border-[var(--border)]">
                    <div className="flex justify-between text-[10px] font-mono text-[var(--text-3)] mb-1">
                        <span className="truncate max-w-[60%]">
                            {truncateFilename(downloadProgress.file)}
                        </span>
            ...
            {isNative && nativeDownloadProgress && !modelReady && (
                <div className="shrink-0 px-5 py-2 bg-[var(--surface-2)] border-b border-[var(--border)]">
                    <div className="flex justify-between text-[10px] font-mono text-[var(--text-3)] mb-1">
                        ...
                        <span>
                            {nativeDownloadProgress.percent > 0 ? `${nativeDownloadProgress.percent}%` : "…"}
                        </span>
                    </div>
            ...
            <div
                ...
                className="flex-1 p-5 h-[380px] bg-[var(--code-bg)] font-mono text-[13px] leading-relaxed text-white/90 overflow-y-auto"
            >
                {error ? (
                    <div className="flex flex-col gap-1.5">
                        <div className="text-red-400 font-sans text-[12px] font-semibold">⚠ Error</div>
                        <div className="text-red-300/80 text-[11px] break-words">
                            {error.message || String(error)}
                        </div>
                        <button
                            onClick={clearError}
                            className="self-start mt-1 text-[11px] text-[var(--text-3)] hover:text-white cursor-pointer transition"
                        >
                            Dismiss
                        </button>
                    </div>
                    ...
                    <div className="flex flex-col items-center justify-center h-full min-h-[140px] text-center gap-3">
                        ...
                        <p className="text-[11px] text-[var(--text-2)] font-sans">Thinking...</p>
                    </div>
                ) : output ? (
                    <div className="select-text break-words">
                        {renderOutput(output, streaming)}
                        {streaming && (
                            <span className="inline-block w-1.5 h-4 bg-[var(--teal)] ml-1 animate-[caret_1s_infinite] align-middle" />
                        )}
                    </div>
                ) : (
            ...
            {(isLocal || isNative || isCloud) && hasStarted && (
                <div
                    className={`transition-opacity duration-300 ${firstTokenArrived ? "opacity-100" : "opacity-0"} shrink-0 px-5 py-3 border-t border-[var(--border)] bg-[var(--surface-2)] flex items-center justify-between select-none font-mono text-[11px]`}
                >
                    {isLocal && (
                        <>
                                    ...
                                    <div className="p-6 flex flex-col gap-4 overflow-y-auto max-h-[70vh]">
                                        <div className="grid grid-cols-3 gap-3">
                                            <div className="bg-[var(--surface-3)] p-3 rounded-xl border border-[var(--border)] flex flex-col">
                                                <span className="text-[10px] text-[var(--text-3)] mb-0.5 uppercase tracking-wider font-semibold">
                                                    Generations
                                                </span>
                                            ...
                                            <div className="bg-[var(--surface-3)] p-3 rounded-xl border border-[var(--border)] flex flex-col">
                                                ...
                                                <span className="text-lg font-bold text-white font-mono">
                                                    {formatSeconds(stats.totalGenMs)}
                                                </span>
                                            </div>
                                        </div>
                                    ...
                                    <div className="p-6 flex flex-col gap-4 overflow-y-auto max-h-[70vh]">
                                        <div className="grid grid-cols-2 gap-4">
                                            ...
                                            <div className="bg-[var(--surface-3)] p-3 rounded-xl border border-[var(--border)] flex flex-col">
                                                ...
                                                <span className="text-lg font-bold text-emerald-400 font-mono">
                                                    {formatCost(stats.totalCacheSavings)}
                                                </span>
                                            </div>
                                        </div>
                                            ...
                                            <div className="grid grid-cols-2 gap-x-4 gap-y-2 text-[12px]">
                                                ...
                                                <div className="flex justify-between col-span-2 border-t border-[var(--border)] pt-2 mt-1">
                                                    ...
                                                    <span className="text-emerald-400 font-mono font-medium">
                                                        {formatCost(stats.avgCostPerGeneration)}
                                                    </span>
                                                </div>
                                            </div>
                                        </div>
                                                    ...
                                                    <table className="w-full text-left font-mono text-[11px] border-collapse">
                                                        <thead>
                                                            <tr className="bg-[var(--surface-3)] text-[var(--text-3)] border-b border-[var(--border)]">
                                                                <th className="p-2 border-r border-[var(--border)]">
                                                                    Provider
                                                                </th>
                                                                <th className="p-2 border-r border-[var(--border)] text-right">
                                                                    Gens
                                                                </th>
                                                                <th className="p-2 border-r border-[var(--border)] text-right">
                                                                    Tokens
                                                        ...
                                                        <tbody>
                                                            {Object.entries(stats.byProvider).map(
                                                                ([prov, pStats]) => (
                                                                    <tr
                                                                        key={prov}
                                                                        className="border-b border-[var(--border)] last:border-b-0 hover:bg-white/5"
                                                                    >
                                                                        <td className="p-2 border-r border-[var(--border)] text-white font-sans capitalize">
                                                                            {prov}
                                                                        </td>
                                                                        <td className="p-2 border-r border-[var(--border)] text-right text-white">
                                                                            {pStats.generationCount}
                                                                        </td>
                                                                        <td className="p-2 border-r border-[var(--border)] text-right text-white">
                                                                            {formatTokens(pStats.totalTokens)}
                                                                        </td>
                                                                        <td className="p-2 text-right text-emerald-400">
                                                                            {formatCost(pStats.totalCost)}
                                                                        </td>
                                                                    </tr>
                                                                )
                                                            )}
            ...
            <div className="shrink-0 border-t border-[var(--border)] bg-[var(--surface-2)] px-5 py-3 flex items-center gap-2">
                {streaming && (
                    <button
                        onClick={cancel}
                        className="inline-flex items-center gap-1.5 cursor-pointer border border-[var(--red)] bg-red-500/10 hover:bg-red-500/20 text-[var(--red)] font-semibold text-[13px] px-3 py-1.5 rounded-xl transition"
                    >
                        Stop
                    </button>
                )}
                {(output || error) && !streaming && (
                    <button
                        onClick={reset}
                        className="text-[12px] font-medium text-[var(--text-2)] hover:text-white px-3 py-1.5 cursor-pointer transition"
                    >
                        Reset
                    </button>
                )}
