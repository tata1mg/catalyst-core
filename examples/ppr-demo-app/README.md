...
A demo app for catalyst-core's three render modes — Streaming, Partial Prerendering (PPR), and Static (full-page cache). This app is the working proof; the section below is the same reference documentation as [`catalyst-core/ppr-documentation.md`](https://github.com/tata1mg/catalyst-core/blob/ppr-poc-v17/ppr-documentation.md), copied here so it reads standalone.
...
All data fetching on a PPR route goes through `usePPRRouteData()`/`DynamicDataProvider` (`catalyst-core`'s `web-router/components/DataFetcher.jsx`):
...
| ------------- | --------- | ------------------------------------------------------------------------------------------------- |
| `/`           | PPR       | A trivial `usePPRRouteData()` greeting, mainly a nav hub to the other demos                        |
| `/ppr-demo`   | PPR       | A static shell (baked into the cached prerender) next to a `usePPRRouteData()` panel that resolves fresh, with a visible delay, on every request                |
| `/dashboard`  | PPR       | A more realistic example — sidebar/header static, all KPI cards/chart/table fed by one combined `usePPRRouteData()` fetch                                        |
| `/pricing`    | Static    | A pricing page (`serverFetcher` + `renderMode = "static"`) — the entire response is rendered once and cached; every later request replays those exact bytes       |

## Running

```bash
npm run start    # dev server, http://localhost:3010
npm run build     # production build
npm run serve     # serve the production build
```

## Observing the render modes

Every response carries an `x-rendering-mode` header. A couple of useful checks:

```bash
# PPR: shell is cached, dynamic panel is fresh every time
curl -sD - http://localhost:3010/ppr-demo -o /dev/null | grep x-rendering-mode   # 1st Req, then resumeToPipeableStream
curl -s http://localhost:3010/ppr-demo | grep -o 'Live order count.*strong>'    # changes every request

# Static: entire page is cached, byte-identical across requests
curl -sD - http://localhost:3010/pricing -o /dev/null | grep x-rendering-mode   # static-cache-miss, then static-cache-hit
diff <(curl -s http://localhost:3010/pricing) <(curl -s http://localhost:3010/pricing)   # empty diff
```

In dev mode (`NODE_ENV !== "production"`), append `?__refresh_static_cache` to `/pricing` to force one request to bypass the cache and re-render — useful while iterating on a static route without restarting the server.

## Documentation

Explore the complete Catalyst documentation at [https://catalyst.1mg.com](https://catalyst.1mg.com).
