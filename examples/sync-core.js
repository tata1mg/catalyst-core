...
function rimraf(dir) {
    ...
    execFileSync(process.execPath, [path.join(REPO_ROOT, "scripts", "safe-rimraf.js"), dir], {
        stdio: "inherit",
    })
}
...
function runWithRetry(cmd, options, beforeRetry) {
    for (let attempt = 1; attempt <= SYNC_MAX_ATTEMPTS; attempt++) {
        try {
            ...
            console.log(
                `\x1b[33m  retrying after transient fs error (attempt ${attempt}/${SYNC_MAX_ATTEMPTS})...\x1b[0m`
            )
