...
export const I = {
    sun: (p = {}) => (
        <span className="material-symbols-outlined" style={{ fontSize: 18, ...p.style }} {...p}>
            light_mode
        </span>
    ),
    moon: (p = {}) => (
    ...
    video: (p = {}) => (
        <span className="material-symbols-outlined" style={{ fontSize: 20, ...p.style }} {...p}>
            videocam
        </span>
    ),
    camera: (p = {}) => (
    ...
    google: () => (
        ...
        >
            ...
            <path
                fill="#34A853"
                d="M9 18c2.43 0 4.47-.8 5.96-2.18l-2.92-2.26c-.8.54-1.83.87-3.04.87-2.34 0-4.32-1.58-5.03-3.7H.95v2.32A9 9 0 0 0 9 18z"
            />
            <path fill="#FBBC05" d="M3.97 10.73a5.42 5.42 0 0 1 0-3.46V4.95H.95a9 9 0 0 0 0 8.1l3.02-2.32z" />
            <path
                fill="#EA4335"
                d="M9 3.58c1.32 0 2.5.45 3.44 1.35l2.58-2.58A9 9 0 0 0 .95 4.95l3.02 2.32C4.68 5.16 6.66 3.58 9 3.58z"
            />
        </svg>
    ),
}
    ...
    return (
        <div className="hookbar" role="status">
                ...
                {state === "loading" ? (
                    ...
                    <span
                        ...
                        style={{
                            ...
                            boxShadow:
                                state === "active"
                                    ? `0 0 0 4px color-mix(in oklab, ${s.color} 22%, transparent)`
                                    : "none",
                        }}
                    />
                )}
            ...
            <span className={`source-badge source-badge--${source}`}>
                {source === "native" ? "Native bridge" : "Web fallback"}
            </span>
        </div>
    )
}
...
export function BottomSheet({ open, onClose, title, children, vtName }) {
    ...
    const sheet = (
        <React.Fragment>
            <div className={`sheet-backdrop ${open ? "sheet-backdrop--open" : ""}`} onClick={onClose} />
            <div
...
export function Lightbox({ open, onClose, title, subtitle, children, footer }) {
    ...
    return ReactDOM.createPortal(
        <React.Fragment>
            <div className={`lightbox-backdrop ${open ? "lightbox-backdrop--open" : ""}`} onClick={onClose} />
            <div
...
export function FileViewer({ file }) {
    ...
    const isText =
        /^(text\/|application\/(json|xml|javascript|x-sh|x-)|application\/yaml)/.test(mime) ||
    ...
    useEffect(() => {
        ...
        file.file
            .text()
            .then((t) => {
                if (!cancelled) setText(t.length > 100_000 ? t.slice(0, 100_000) + "\n…[truncated]" : t)
            })
            .catch((e) => {
                if (!cancelled) setErr(e?.message || "read error")
            })
        return () => {
            cancelled = true
        }
        ...
        <div className="lightbox__unknown">
            ...
            <div>
                ...
                <div style={{ fontSize: 12, opacity: 0.7, marginTop: 4 }}>
                    {mime || "unknown"} · {formatBytes(file.size)}
                </div>
            </div>
            <a
                href={file.url}
