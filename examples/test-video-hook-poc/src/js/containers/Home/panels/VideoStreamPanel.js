...
export function VideoStreamPanel() {
    ...
    const {
        ...
        webFallbackActive,
        mediaStream,
    } = useVideoStream({ onQRDetected, webFallback: fallback })
        ...
        <div className="col">
            ...
            {format !== "barcode" && (
                <div className="card">
                    ...
                    <div
                        className="card__val card__val--mono"
                        style={{
                            textAlign: "left",
                            color: qr ? "var(--accent-green)" : "var(--text-secondary)",
                        }}
                    >
                        {qr || "No scan yet"}
                    </div>
            ...
            {isStreaming && (
                <div className="card col" style={{ gap: 12 }}>
                    {restartNote && (
                        <div
                            className="help"
                            style={{ color: "var(--accent-amber)", marginTop: 0, fontWeight: 500 }}
                        >
                    ...
                    <div>
                        ...
                        {editingFps ? (
                            ...
                            <span className="pill pill--blue">
                                {streamState?.fps?.min || fpsMin || "—"} –{" "}
                                {streamState?.fps?.max || fpsMax || "—"} fps
                            </span>
                        )}
                    </div>
                    ...
                    <div className="row">
                        ...
                        <button className="btn grow" onClick={handleFlip}>
                            {I.flip()} Flip
                        </button>
                    </div>
                </div>
            ...
            <div className="row" style={{ justifyContent: "center", padding: "4px 0 0" }}>
                {isStreaming ? (
                    <button className="round-btn round-btn--stop" onClick={handleStop} aria-label="Stop">
                        <span
                            style={{
                                width: 18,
                                height: 18,
                    ...
                    <button className="round-btn" onClick={() => handleStart()} aria-label="Start">
                        <span
                            style={{
                                width: 22,
                                height: 22,
            ...
            <div className="col" style={{ gap: 12 }}>
                <div>
                    ...
                    <div className="btn-toggle">
                        <button
                            className={`btn-toggle__item ${facing === "back" ? "btn-toggle__item--active" : ""}`}
                            onClick={() => setFacing("back")}
                        >
                            Back
                        </button>
                        <button
                            className={`btn-toggle__item ${facing === "front" ? "btn-toggle__item--active" : ""}`}
                ...
                <div>
                    ...
                    <div className="seg">
                        {["qr", "barcode", "all"].map((f) => (
                            <button
                                key={f}
                                className={`seg__item ${format === f ? "seg__item--active" : ""}`}
                                onClick={() => setFormat(f)}
                            >
                                {f.toUpperCase()}
                            </button>
                    ...
                    <div className="row" style={{ gap: 8 }}>
                        ...
                        <div className="col" style={{ flex: 1, gap: 4 }}>
                            ...
                            <input
                                ...
                                placeholder="—"
                                value={fpsMax}
                                onChange={(e) => setFpsMax(e.target.value)}
                            />
                        </div>
                    </div>
                    <div className="help">
                        Leave empty to let the device choose. Live edits use sendCommand(&apos;fps&apos;).
                    </div>
                </div>
