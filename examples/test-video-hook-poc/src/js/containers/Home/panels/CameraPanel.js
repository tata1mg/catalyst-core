...
export function CameraPanel() {
    ...
    useEffect(() => {
        if (data) {
            const dataUrl =
                data.fileSrc || data.uri || `data:${data.mimeType || "image/jpeg"};base64,${data.base64}`
    ...
    return (
        <div className="col">
            ...
            <div
                ...
                style={{ height: 220, cursor: latest ? "pointer" : "default" }}
                onClick={() => latest && setViewing(latest)}
            >
                {latest ? (
            ...
            <Lightbox
                ...
                subtitle={
                    viewing
                        ? `${viewing.type} · ${formatBytes(viewing.size)} · ${viewing.ts.toLocaleTimeString()}`
                        : ""
                }
                footer={
                    viewing && (
