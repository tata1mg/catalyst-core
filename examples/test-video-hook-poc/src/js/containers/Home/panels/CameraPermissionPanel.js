...
export function CameraPermissionPanel() {
    ...
    const {
        ...
        error,
        request,
    } = useCameraPermission({ webFallback: fallback })
    const { push } = useToast()
    ...
    const cls =
        perm === "granted" ? "state--granted" : perm === "denied" ? "state--denied" : "state--undetermined"
    ...
    return (
        <div className="col">
            ...
            {webFallbackActive && (
                <button className="btn btn--block btn--primary" onClick={request} disabled={isLoading}>
                    Request Permission
                </button>
            )}
        </div>
