...
export function HapticPanel() {
    ...
    const { light, medium, heavy, success, warning, errorHaptic, selection, isSupported, webFallbackActive } =
        useHapticFeedback({ webFallback: fallback })
    ...
    const cell = (kind, label, sub, fn) => (
        <div
            className={`haptic-cell ${fired === kind ? "haptic-cell--fired" : ""}`}
    ...
    return (
        <div className="col">
            ...
            />
            <HookStatusBar
                state={fired ? "active" : "idle"}
                label={fired ? `Fired: ${fired}` : "Idle"}
