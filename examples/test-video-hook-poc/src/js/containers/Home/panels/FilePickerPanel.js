...
export function FilePickerPanel() {
    ...
    const { data, loading, error, execute, webFallbackActive, clear, getFileObject, getFileObjects } =
        hookReturn
    ...
    useEffect(() => {
        if (data) {
            ...
            setFiles((prevFiles) => {
                return list.map((f, i) => {
                    const existing = prevFiles.find(
                        (x) => x.fileName === (f.fileName || f.name) && x.size === (f.fileSize || f.size)
                    )
    ...
    const transportClass = (t) =>
        t === "OBJECT_URL" ? "pill--blue" : t === "BRIDGE_BASE64" ? "pill--orange" : "pill--green"
    ...
    return (
        <div className="col">
            ...
            />
            <HookStatusBar
                state={loading ? "loading" : files.length ? "active" : error ? "error" : "idle"}
                label={files.length ? `${files.length} file${files.length === 1 ? "" : "s"}` : "Idle"}
                    ...
                    <div className="field">
                        <div className="field__label">
                            ...
                            <div className="field__hint">
                                e.g. <span className="kbd">image/*</span>,{" "}
                                <span className="kbd">application/pdf</span>
                            </div>
                        </div>
                        <input
            ...
            {files.length > 0 && (
                <div className="card" style={{ padding: 0 }}>
                    <div className="spread" style={{ padding: "8px 12px 4px" }}>
                        <div className="card__label" style={{ margin: 0 }}>
                            Selected files ({files.length})
                        </div>
                        <button
                            className="btn btn--ghost"
                            style={{ height: 28, padding: "0 8px", color: "var(--accent-red)", fontSize: 13 }}
                            onClick={handleClear}
                        >
                            Clear all
                        </button>
                    ...
                    {getFileObjects && files.length > 1 && (
                        <div style={{ padding: "8px 12px 12px" }}>
                            <button className="btn btn--block" onClick={handleGetAllFileObjects}>
                                Get All as File Objects
                            </button>
                        </div>
                    )}
                </div>
            ...
            <Lightbox
                ...
                subtitle={
                    viewing ? `${viewing.mimeType} · ${formatBytes(viewing.size)} · ${viewing.transport}` : ""
                }
            >
                {viewing && (
                    <FileViewer
