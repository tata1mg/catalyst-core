...
const VERSION_GATES = {
    ...
    T8_CLIENT_ENTRY: {
        ...
        upgradeNote:
            "Update catalyst-core to ^0.1.0-canary.1 or later and run npm install. Also ensure client/index.js follows the new template pattern.",
    },
}
...
function handle_get_conversion_status({ project_path, include_not_applicable = false } = {}) {
    ...
    const detectors = {
        ...
        T1_CONFIG: () => {
            if (!config)
                return {
                    status: "gap",
                    native_risk: "App will not start — catalyst-core requires config/config.json at boot.",
                    reason: "config/config.json not found",
                }
            const missing = ["NODE_SERVER_PORT", "WEBVIEW_CONFIG", "API_URL"].filter(
                (k) => config[k] === undefined
            )
            if (missing.length)
                return {
                    status: "gap",
                    native_risk: `Missing required fields will cause build or runtime failures: ${missing.join(", ")}.`,
        ...
        T3_ROUTES_FILE: () => {
            ...
            if (!text)
                return {
                    status: "gap",
                    native_risk:
                        "catalyst-core expects a static route array for SSR. Without it, server-side rendering fails and native app shell cannot hydrate.",
                    reason: "src/js/routes/index.js not found",
                }
            const hasArray =
                /const\s+\w+\s*=\s*\[[\s\S]*?\]\s*\n[\s\S]*export default\s+\w+|export default\s*\[|module\.exports\s*=\s*\[/.test(
                    text
                )
            if (!hasArray)
                return {
        ...
        T4_DATA_FETCHING: () => {
            ...
            return {
                status: "needs_review",
                native_risk:
                    "useEffect+fetch for page data works on web but breaks SSR hydration in catalyst-core. Native app will have blank pages or hydration mismatches on first load.",
                ...
                review_context: {
                    question:
                        "Do the files using useEffect for data fetching represent unconverted page-level data loading, or are they legitimate non-page effects (timers, subscriptions, animations)?",
                    what_correct_looks_like:
                        "Page components have .serverFetcher or .clientFetcher static functions attached. useEffect is only used for side effects (subscriptions, DOM mutations, timers), not initial data loads.",
                    what_gap_looks_like:
                        "Page components (in src/js/pages/ or src/js/containers/) call fetch/axios inside useEffect to load initial data, with no .serverFetcher or .clientFetcher attached.",
                    signal_files: {
        ...
        T5_ROUTER_DATA_PROVIDER: () => {
            ...
            if (!text)
                return {
                    status: "gap",
                    native_risk:
                        "Without RouterDataProvider, useCurrentRouteData() returns undefined in all components. Data-dependent pages will crash.",
                    reason: "src/js/routes/utils.js not found",
                }
            if (!text.includes("RouterDataProvider"))
                return {
                    status: "gap",
                    native_risk:
                        "utils.js exists but RouterDataProvider not wired in — same crash risk as missing file.",
                    reason: "utils.js exists but RouterDataProvider not used",
                }
            return { status: "completed" }
        },
        ...
        T6_APP_SHELL: () => {
            ...
            if (!text)
                return {
                    status: "gap",
                    native_risk:
                        "App shell is required by catalyst-core SSR. Missing file means native app renders blank screen.",
                    reason: "src/js/containers/App/index.js not found",
                }
            if (!text.includes("Outlet"))
                return {
                    status: "gap",
                    native_risk:
                        "<Outlet /> is how catalyst-core injects SSR-rendered route content. Without it, all pages render empty inside the native shell.",
                    reason: "App/index.js exists but <Outlet /> not rendered",
                }
            return { status: "completed" }
        },

        T7_SERVER_FILES: () => {
            const missing = ["server/index.js", "server/server.js", "server/document.js"].filter(
                (f) => !fileExists(f)
            )
            if (missing.length)
                return {
                    status: "gap",
                    native_risk:
                        "Missing server files prevent catalyst-core SSR from running. Native app will fail at boot with module-not-found errors.",
        ...
        T9_WEBVIEW_ANDROID: () => {
            if (!webviewConfig)
                return {
                    status: "gap",
                    native_risk: "No WEBVIEW_CONFIG — Android build cannot start.",
                    reason: "WEBVIEW_CONFIG missing (see T1_CONFIG)",
                }
            ...
            if (!android)
                return {
                    status: "gap",
                    native_risk:
                        "WEBVIEW_CONFIG.android block missing — catalyst-core cannot generate the Android project.",
                    reason: "WEBVIEW_CONFIG.android block missing",
                }
            const missing = ["buildType", "sdkPath", "emulatorName", "appName"].filter(
                (k) => android[k] === undefined
            )
            if (missing.length)
                return {
                    status: "gap",
                    native_risk: `Android build will fail or produce unnamed app. Missing: ${missing.join(", ")}.`,
                    reason: `WEBVIEW_CONFIG.android missing fields: ${missing.join(", ")}`,
                }
            return { status: "completed" }
        },
        ...
        T10_WEBVIEW_IOS: () => {
            if (!webviewConfig)
                return {
                    status: "gap",
                    native_risk: "No WEBVIEW_CONFIG — iOS build cannot start.",
                    reason: "WEBVIEW_CONFIG missing (see T1_CONFIG)",
                }
            ...
            if (!ios)
                return {
                    status: "gap",
                    native_risk:
                        "WEBVIEW_CONFIG.ios block missing — catalyst-core cannot generate the Xcode project.",
                    reason: "WEBVIEW_CONFIG.ios block missing",
                }
            const missing = ["buildType", "appBundleId", "simulatorName", "appName"].filter(
                (k) => ios[k] === undefined
            )
            if (missing.length)
                return {
                    status: "gap",
                    native_risk: `iOS build will fail or be rejected by App Store. Missing: ${missing.join(", ")}.`,
                    reason: `WEBVIEW_CONFIG.ios missing fields: ${missing.join(", ")}`,
                }
            return { status: "completed" }
        },
        ...
        T11_ACCESS_CONTROL: () => {
            if (!webviewConfig)
                return {
                    status: "gap",
                    native_risk: "No WEBVIEW_CONFIG — access control cannot be configured.",
                    reason: "WEBVIEW_CONFIG missing (see T1_CONFIG)",
                }
            ...
            if (!ac)
                return {
                    status: "gap",
                    native_risk:
                        "Without accessControl, the WebView allows navigation to any URL. Deep links or redirects could take users outside your app with no way back.",
                    reason: "WEBVIEW_CONFIG.accessControl block missing",
                }
            if (!ac.enabled)
                return {
                    status: "gap",
                    native_risk:
                        "accessControl.enabled=false means URL allowlist is ignored — same risk as missing block.",
                    reason: "accessControl.enabled is not true",
                }
            if (!Array.isArray(ac.allowedUrls) || ac.allowedUrls.length === 0)
                return {
                    status: "gap",
                    native_risk:
                        "allowedUrls is empty — ALL navigation URLs will be blocked including your own API calls. App will appear to hang on every network request.",
                    reason: "accessControl.allowedUrls is empty — ALL URLs will be blocked",
                }
            return { status: "completed" }
        },
        ...
        T12_SPLASH_SCREEN: () => {
            if (!config)
                return {
                    status: "gap",
                    native_risk: "No config — splash screen config cannot be read.",
                    reason: "config/config.json not found",
                }
            if (!config.splashScreen)
                return {
                    status: "gap",
                    native_risk:
                        "Without splashScreen config, native app shows a blank white screen during JS load. Not a crash, but poor UX and noticeable on slow devices.",
                    reason: "splashScreen key missing from top level of config/config.json (not inside WEBVIEW_CONFIG)",
                }
            const missing = ["public/android/splashscreen.png", "public/ios/splashscreen.png"].filter(
                (f) => !fileExists(f)
            )
            if (missing.length)
                return {
                    status: "gap",
                    native_risk:
                        "Splash config exists but asset files missing — build will fail when packaging the native app.",
                    reason: `Splash asset files missing: ${missing.join(", ")}`,
                }
            return { status: "completed" }
        },

        T13_ANDROID_ICONS: () => {
            const required = ["mdpi", "hdpi", "xhdpi", "xxhdpi", "xxxhdpi"].map(
                (d) => `public/android/appIcons/icon-${d}.png`
            )
            ...
            if (missing.length)
                return {
                    status: "gap",
                    native_risk:
                        "Missing Android icon densities — build will fall back to bundled catalyst icon. Expected naming: icon-mdpi.png, icon-hdpi.png, icon-xhdpi.png, icon-xxhdpi.png, icon-xxxhdpi.png inside public/android/appIcons/.",
                    reason: `Missing Android icons: ${missing.join(", ")}`,
                }
            return { status: "completed" }
        },
        ...
        T14_IOS_ICONS: () => {
            ...
            if (missing.length)
                return {
                    status: "gap",
                    native_risk:
                        "Missing iOS icon sizes — Xcode build may fail or App Store submission will be rejected. Expected naming: icon-{WxH}-{scale}.png e.g. icon-60x60-3x.png inside public/ios/appIcons/.",
                    reason: `Missing iOS icons: ${missing.join(", ")}`,
                }
            return { status: "completed" }
        },
        ...
        T15_OFFLINE_HTML: () => {
            if (!fileExists("public/offline.html"))
                return {
                    status: "gap",
                    native_risk:
                        "Without offline.html, the native WebView shows a system error page when the device goes offline. Bad UX; should show a branded offline screen.",
                    reason: "public/offline.html not found",
                }
            return { status: "completed" }
        },
        ...
        T17a_USE_FILEPICKER: () => {
            ...
            return {
                status: "needs_review",
                native_risk:
                    '<input type="file"> may work in some WebView configs but behaviour is inconsistent across Android/iOS versions. On Android, file picker may silently do nothing without proper WebView flags. useFilePicker guarantees native picker on both platforms.',
                ...
                review_context: {
                    question:
                        "Do files with <input type='file'> have a native branch using useFilePicker, or is the hook used somewhere in the component tree (parent passes execute/isNative as props)?",
                    what_correct_looks_like:
                        "Component uses useFilePicker from catalyst-core/hooks. Checks isNative and calls execute() on native. May keep <input type='file'> as web fallback only. Hook may live in a parent and be passed down as props.",
                    what_gap_looks_like:
                        "<input type='file'> used directly with no useFilePicker anywhere in the component tree. No isNative check. No native branch.",
                    signal_files: {
        ...
        T17b_USE_CAMERA: () => {
            ...
            return {
                status: "needs_review",
                native_risk:
                    "Camera capture via <input capture> does NOT work reliably in native WebView. On iOS it may trigger the permission flow but fail silently. On Android it often does nothing. useCamera is required for guaranteed camera access with proper permission handling.",
                reason: `${oldPatternFiles.length} file(s) with camera capture inputs. Review: does useCamera exist in the component tree?`,
                review_context: {
                    question:
                        "Do files with <input accept='image/*' capture> have a native branch using useCamera, or is the hook in the component tree?",
                    what_correct_looks_like:
                        "Component uses useCamera from catalyst-core/hooks. Checks isNative and calls takePhoto() on native. May keep capture input as web fallback.",
                    what_gap_looks_like:
                        "<input capture> used with no useCamera in component tree. No isNative check. No native camera branch.",
                    signal_files: {
                        old_pattern_files: oldPatternFiles,
                        hook_usage_files: hookUsageFiles,
                    },
        ...
        T18_USE_HAPTIC: () => {
            ...
            return {
                status: "needs_review",
                native_risk:
                    "navigator.vibrate() is a web API that silently does nothing inside native WebView on both Android and iOS. The vibration call succeeds but the device never vibrates. useHapticFeedback routes to the native haptic engine.",
                ...
                review_context: {
                    question:
                        "Are navigator.vibrate() calls unconverted haptic triggers, or are they in test files, comments, or polyfills?",
                    what_correct_looks_like:
                        "useHapticFeedback imported from catalyst-core/hooks. trigger() called with type string. navigator.vibrate only appears in web polyfills (if at all).",
                    what_gap_looks_like:
                        "navigator.vibrate() called directly in component or utility code with no useHapticFeedback import.",
                    signal_files: {
                        old_pattern_files: oldPatternFiles,
                        hook_usage_files: hookUsageFiles,
                    },
        ...
        T19_USE_NOTIFICATIONS: () => {
            if (!webviewConfig || !webviewConfig.notifications || !webviewConfig.notifications.enabled) {
                ...
                if (pushFiles.length === 0)
                    return {
                        status: "not_applicable",
                        reason: "notifications.enabled not set and no push notification code found.",
                    }
                return {
                    status: "gap",
                    native_risk:
                        "Project appears to use push notifications but notifications.enabled is not set in WEBVIEW_CONFIG. Firebase integration will not be wired into the native build — push notifications will silently not work.",
            ...
            if (missing.length)
                return {
                    status: "gap",
                    native_risk:
                        "notifications.enabled=true but Firebase config files missing — native build will fail at compile time.",
                    reason: `notifications.enabled=true but Firebase files missing: ${missing.join(", ")}`,
                }
            return { status: "completed" }
        },
        ...
        T20_USE_DEVICE_INFO: () => {
            ...
            return {
                status: "needs_review",
                native_risk:
                    "navigator.userAgent inside native WebView returns the WebView UA string, not Android/iOS. UA-sniffing for platform branching gives wrong results — features guarded by /Android/i.test() may never activate on native.",
                ...
                review_context: {
                    question:
                        "Are navigator.userAgent patterns used for platform detection in component/utility code, or in analytics, SSR utilities, or third-party code where UA is acceptable?",
                    what_correct_looks_like:
                        "Platform detection uses isNative from catalyst hooks (e.g. const { isNative } = useNetworkStatus()), or window.NativeBridge presence for imperative code. navigator.userAgent only in analytics or server-side code.",
                    what_gap_looks_like:
                        "Component or utility code branches UI/behavior on navigator.userAgent. Unreliable inside WebView.",
                    signal_files: {
                        old_pattern_files: oldPatternFiles,
    ...
    function runTask(id) {
        ...
        results[id] = detector
            ? detector()
            : { status: "gap", native_risk: "Unknown", reason: "No detector implemented" }
        return results[id]
    }
        ...
        switch (r.status) {
            ...
            case "needs_review":
                needs_review.push({
                    ...
                    tier: task.tier,
                    title: task.title,
                    native_risk: r.native_risk,
                    reason: r.reason,
                    review_context: r.review_context,
                    fix_guide: task.fix_guide,
                    verify_hint: task.verify_hint || null,
    ...
    try {
        topLevelFiles = fs
            .readdirSync(root)
            .filter((f) => !["node_modules", ".git", "build", "dist"].includes(f))
    } catch {
        // Best-effort project root scan; keep empty context on failure.
    }
    let srcStructure = []
    try {
        srcStructure = fs.readdirSync(path.join(root, "src"))
    } catch {
        // src/ may not exist yet in partially converted projects.
    }
...
function handle_get_conversion_tasks({ project_path, filter = "all", include_not_applicable = false } = {}) {
    ...
    const blockedList = applyFilter(status.blocked).map((b) => {
        ...
        return {
            ...
            title: task.title,
            status: "blocked",
            reason: `Blocked by ${b.blocked_by} — fix that first`,
            files: null,
            fix_guide: task.fix_guide,
            how_to_check: task.how_to_check,
            depends_on: task.depends_on,
